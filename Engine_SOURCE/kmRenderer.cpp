...
namespace km::renderer
{
	...
	void LoadBuffer()
	{
		...
		km::graphics::GetDevice()->CreateBuffer(&triangleBuffer, &triangleDesc, &triangleData);
		...
		km::graphics::GetDevice()->CreateBuffer(&triangleIdxBuffer, &triangleIdxDesc, &triangleIdxData);
		...
		km::graphics::GetDevice()->CreateBuffer(&triangleConstantBuffer, &triangleCSDesc, nullptr);

		Vector4 pos(0.3f, 0.0f, 0.0f, 1.0f);

		km::graphics::GetDevice()->SetConstantBuffer(triangleConstantBuffer, &pos, sizeof(Vector4));
		km::graphics::GetDevice()->BindConstantBuffer(eShaderStage::VS, eCBType::Transform, triangleConstantBuffer);
	}

	void LoadShader()
	{
		km::graphics::GetDevice()->CreateShader();
	}
